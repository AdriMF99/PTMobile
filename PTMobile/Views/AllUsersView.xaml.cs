...
public partial class AllUsersView : ContentPage
{
    ...
    private async void OnImageTapped(object sender, EventArgs e)
    {
        var userName = (string)((TappedEventArgs)e).Parameter;

        if (BindingContext is AllUsersViewModel viewModel)
        {
            await viewModel.ImageTappedCommand.ExecuteAsync(userName);
        }
    }
    ...
    private async void OnGodTapped(object sender, EventArgs e)
    {
        ...
        if (BindingContext is AllUsersViewModel viewModel)
        {
            await viewModel.GodTappedCommand.ExecuteAsync(userName);
        }
    }
}